...
function makeBringItBackScene(): Scene {
  ...
  const beats: VocabularyBeat[] = [
    ...
    },
    {
      ...
      practice: {
        ...
        successStateChanges: [
          ...
          sceneStateChanges.setVariant(heroId, 'playful'),
        ],
      },
    },
    {
      key: 'playful',
      meaningVi: 'tinh nghịch và thích chơi',
      word: 'playful',
      tier: 'expanded',
      type: 'adjective',
      speechPractice: 'optional',
      cueObjectId: heroId,
      teachVi: 'Chạm chú cún đang cúi hai chân trước mời chơi nhé.',
      teachEn: 'Tap the playful puppy making a play bow.',
      teachSuccessEn:
        'Playful means eager to play; a dog may show it with a play bow.',
      teachFailVi: 'Chạm chú cún đang cúi thấp hai chân trước nhé.',
      teachFailEn: 'Tap the puppy with its front legs lowered.',
      practice: {
        instructionVi: 'Chạm cún đang cúi chào để chơi thêm nhé.',
        instructionEn: 'Tap the puppy making a play bow to play again.',
        successVi: 'Tư thế cúi chào cho thấy cún muốn chơi thêm.',
        successEn: 'The playful puppy invites you to play another turn.',
        failVi: 'Chạm chú cún đang cúi hai chân trước nhé.',
        failEn: 'Tap the puppy making a play bow.',
        targetObjectId: heroId,
        effects: [lessonEffects.sound('yay')],
        successStateChanges: [sceneStateChanges.setVariant(heroId, 'playful')],
      },
    },
    {
      key: 'your-turn',
    ...
    },
    {
      ...
      teachEn: 'Tap the playful puppy ready for another turn.',
      teachSuccessEn:
  ...
  const vocabulary = makeBeatVocabulary(sceneId, beats);
  const playfulVocabulary = vocabulary.find(item => item.word === 'playful')!;
    ...
    objects: [
      sceneObject({
        ...
        variants: [
          ...
          objectVariant({
            id: 'playful',
            assetSource: sceneImageSource('choose-the-ball', 'puppy-play-bow'),
            position: rect(59, 44, 36, 35),
          }),
        ],
      }),
      sceneObject({
      ...
      learningObject({
        id: `${sceneId}-playful-representative`,
        assetSource: sceneImageSource('choose-the-ball', 'puppy-play-bow'),
        initialVisibility: 'hidden',
        isInteractive: false,
        learningScope: expandedScope,
        position: rect(59, 44, 36, 35),
        vocab: playfulVocabulary,
      }),
      ...makeBeatObjects(sceneId, beats, vocabulary),
    ],
    steps: [
