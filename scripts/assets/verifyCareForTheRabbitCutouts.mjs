...
const masterRoot = join(
  repoRoot,
  'src/assets/source/master/lessons/care-for-the-rabbit',
);
const expectedAssets = {
  'prepare-the-hay': [
    ...
    'rabbit-looking-rack',
    'rabbit-chewing-hay',
    'rabbit-standing-calm',
    'hay-bundle',
    'hay-rack-empty',
    'hay-rack-full',
    'rabbit-hutch',
    'rabbit-chewing-closeup',
    'fresh-hay-pile',
    'fill-hay-rack-action',
  ],
  'fill-the-water': [
    'background',
    'rabbit-thirsty',
...
if (errors.length > 0) {
  console.error('Care-for-the-rabbit cutout audit failed:');
  for (const error of errors) console.error(`- ${error}`);
  process.exitCode = 1;
} else {
  console.log(`Care-for-the-rabbit cutout audit passed (${count} masters).`);
}
