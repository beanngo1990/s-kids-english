    ...
    ...uiTargets.map(
      target =>
        `  ${formatString(target.key)}: require(${formatString(
          `../assets/${getBundledUiAudioRequireKey(target.key)}`,
        )}),`,
    ),
    '};',
    '',
...
function getBundledUiAudioRequireKey(targetKey) {
  const optimizedKey = targetKey.replace(/\.wav$/u, '.mp3');
  return existsSync(join(repoRoot, 'src/assets', optimizedKey))
    ? optimizedKey
    : targetKey;
}
