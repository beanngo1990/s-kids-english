# Theme 4 Storyboard - Garden Friends
...
- `descriptionVi`: `Bé quan sát giun, ốc sên, ong, bướm và những bạn nhỏ quanh cây.`
- `descriptionEn`: `Observe earthworms, snails, bees, butterflies, and other little garden neighbors.`
...
thành câu chuyện. Các con vật được gọi là bạn/hàng xóm trong vườn; copy không khẳng định mọi loài
đều trực tiếp giúp cây.

## 3. Scene `under-the-leaf`
...
- Hero plant starts with one open yellow flower; variant after time cue has one tiny green fruit.
- `bee`, `butterfly` and their observation rings start hidden.
- Two watch controls reveal visitors one at a time; child never taps the animals.

### Step order

1. Intro: `Vài ngày sau, nụ hoa đã nở.`
2. Teach `flower`: `Chạm bông hoa màu vàng bên trái nhé.`
3. Tap first watch control: `Chạm kính lúp dưới bên phải để chờ ong nhé.` -> bee visible.
...
## 5. Scene `quiet-garden-watch`

### State và object

- Caterpillar and observation ring start hidden behind a leaf edge.
...
`background`, `hero-plant`, `leaf-cover-closed`, `leaf-cover-lifted`, `leaf-cover-replaced`,
`earthworm`, `snail`, `observation-ring`, `soil-patch`, `tunnel`, `look-under-leaf-action`,
`look-over-flower-action`.

### `flower-visitors`

`background`, `plant-flower`, `plant-tiny-fruit`, `flower`, `bee`, `butterfly`,
`observation-ring`, `watch-control-low`, `watch-control-high`, `time-cue`.

### `quiet-garden-watch`

`background`, `hero-plant`, `leaf-tip`, `leaf-tip-lifted`, `caterpillar`, `observation-ring`, `birdbath-empty`,
`birdbath-filled`, `water-drop`, `quiet-hands-control`, `garden-neighbors`,
`watch-gently-action`, `wave-hands-action`.

Bundled map icons: `under-the-leaf`, `flower-visitors`, `quiet-garden-watch`, and
`milestone-garden-friends`. They must be distinct from every existing scene/milestone icon.
...
5. State changes reset safely per scene and do not imply cross-scene persistence.
6. Animal visits and tiny-fruit payoff respect the time cue and safety copy.
7. Production PNG masters have alpha, transparent corners, no chroma residue and no baked text.
8. Lesson validation, image audit/build/verify and Android build pass before release; full Jest
   may retain only a documented, unrelated baseline failure.

## 8. Release status

- Đợt Google TTS đầu tiên đã tạo 257 file mới và publish cùng ảnh. Action-first copy sau đó tạo
  thêm đúng 85 file; pipeline hiện resolve 343 logical audio targets với `Missing files: 0`,
  `Invalid files: 0`.
- Images: 35 WebP production assets.
