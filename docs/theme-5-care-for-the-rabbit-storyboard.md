# Theme 5 Storyboard — Care for the Rabbit
...
- Cỏ khô (`hay`) và nước (`water`) là thức ăn chính thiết yếu; cà rốt (`carrot`) chỉ là món ăn dặm/thưởng nhỏ do người lớn chuẩn bị.
- Tuyệt đối an toàn: không kéo tai thỏ, không bế xốc hay ôm chặt, kết thúc bằng việc rửa tay sau khi chăm sóc thú cưng.
...
Mỗi scene có đúng 3/6/9 từ, 6/12/18 hành động và 3/4/5 lượt auto-micro theo Core/Expanded/Challenge. Mỗi scene có 1 intro listen-only mở đầu (tổng step HUD là 7/13/19).

---

## 2. Story and vocabulary

### Scene 1 — `prepare-the-hay` (Chuẩn bị cỏ khô)

**Mục tiêu câu chuyện:** Bé làm quen bạn thỏ trắng, nhận biết thỏ đang đói và chuẩn bị cỏ khô thơm ngon cho vào máng cỏ.
...
| | `fill the rack` | "Bé chạm để cho đầy cỏ vào máng nhé!" | Chạm máng cỏ -> Cỏ khô được cho đầy vào máng. |
| | `gentle rabbit` | "Bé chạm vào chú thỏ ngoan đang đứng đợi nào!" | Chạm thỏ -> Thỏ đứng yên hiền lành bên máng cỏ đầy. |

**State transition chính:**
```text
rabbit: sitting-waiting -> looking-at-rack -> chewing-hay
hay-rack: empty -> filling -> full-of-hay
```

---

### Scene 2 — `fill-the-water` (Thêm nước sạch)

**Mục tiêu câu chuyện:** Kiểm tra bát nước của thỏ, rót nước sạch mát và đặt vào chuồng để thỏ uống nước.

| Tier | Vocabulary | Lời thoại tiếng Việt (Audio prompt) | Thao tác & Phản hồi hình ảnh (Visual Payoff) |
| :--- | :--- | :--- | :--- |
| **Core** | `water` | "Bé chạm vào bình nước sạch mát nào!" | Chạm bình nước -> Dòng nước trong lành sẵn sàng rót. |
| | `bowl` | "Bé tìm chiếc bát nhỏ của thỏ nhé!" | Chạm bát nước -> Bát nước đặt ngay ngắn trên thảm. |
...
Review allow-list gồm 6 anchor targets theo thứ tự:
```text
1. vocab-care-for-the-rabbit-prepare-the-hay-rabbit
2. vocab-care-for-the-rabbit-prepare-the-hay-hay
3. vocab-care-for-the-rabbit-fill-the-water-water
4. vocab-care-for-the-rabbit-rabbit-snack-and-hop-hop
5. vocab-care-for-the-rabbit-prepare-the-hay-hay-rack
6. vocab-care-for-the-rabbit-rabbit-snack-and-hop-feed-the-rabbit
```

Executable pool tương ứng từng mode:
- **Core (4 items):** `rabbit`, `hay`, `water`, `hop`.
- **Expanded (5 items):** `rabbit`, `hay`, `water`, `hop`, `hay rack`.
- **Challenge (6 items):** `rabbit`, `hay`, `water`, `hop`, `hay rack`, `feed the rabbit`.

**Đặc điểm nhận diện hình ảnh (Visual Silhouettes) của 6 Review Items:**
1. `rabbit`: Chú thỏ trắng tai dài ngồi ngoan ngoãn.
2. `hay`: Búi cỏ khô màu xanh ngà bó gọn gàng.
3. `water`: Dòng nước mát / bát nước trong vắt.
4. `hop`: Chú thỏ bật nhảy trên không kèm vệt chuyển động vui nhộn.
5. `hay rack`: Chiếc máng cỏ bằng gỗ sạch sẽ.
6. `feed the rabbit`: Bàn tay đưa mẩu thức ăn cho thỏ một cách nhẹ nhàng.
...
- **Đồ họa Cutout:** 
  ...
  - Vị trí các đồ vật (máng cỏ, bát nước, thỏ, khay thức ăn) được phân bổ hitbox rõ ràng, không đè lên nhau gây khó khăn khi thao tác.
- **An toàn chăm sóc thú cưng (Pet Care Safety):**
  - Nhắc nhở rõ ràng rằng thỏ cần cỏ khô và nước sạch mỗi ngày; cà rốt chỉ là món thưởng nhỏ do người lớn chuẩn bị.
...
1. Đạt chính xác ngân sách từ vựng (9 Core / 18 Expanded / 27 Challenge) và hành động (18 / 36 / 54).
2. Phân bổ lượt mở micro tự động đúng chuẩn (9 Core / 12 Expanded / 15 Challenge; 3/4/5 mỗi scene).
